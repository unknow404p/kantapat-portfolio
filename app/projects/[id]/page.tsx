...
export default async function ProjectDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <>
      {/* -----------------------------------------------------------
          LAYER 1: ปุ่มควบคุม (แยกออกมาไว้นอกสุด ลอยเหนือทุกอย่าง)
          ต้องอยู่นอก Container หลัก เพื่อไม่ให้ติดกับดัก Z-Index
      ----------------------------------------------------------- */}
    {/* -------------------------------------------------------------
          ปุ่ม BACK TO PROJECTS (แบบเห็นชัดทันที)
          - ปกติ: ขอบดำ ตัวหนังสือดำ พื้นขาว
          - Hover: สลับสีเป็นพื้นดำ ตัวหนังสือขาว
      ------------------------------------------------------------- */}
      <Link 
        href="/projects" 
        style={{ zIndex: 999999 }} 
        className="fixed top-6 left-6 z-[99999] group inline-flex items-center justify-center px-8 py-3 rounded-full 
      ...
      >
        <div className="flex items-center gap-3">
           ...
           {/* ข้อความ */}
           <span className="font-black text-sm tracking-widest uppercase">BACK TO PROJECTS</span>
        </div>
      </Link>

      {/* ปุ่ม Live Preview ก็แยกออกมาเหมือนกัน */}
      <div style={{ zIndex: 999999 }} className="fixed top-8 right-6 hidden md:flex items-center gap-3 bg-white/50 px-4 py-2 rounded-full border border-black/5 backdrop-blur-md">
      ...
      {/* -----------------------------------------------------------
          LAYER 2: เนื้อหาหลัก (อยู่ใต้ Navbar)
          ใช้ z-40 เพื่อให้ Global Navbar (ที่มักเป็น z-50) ลอยทับพื้นหลังได้
      ----------------------------------------------------------- */}
      <div className="fixed inset-0 z-40 overflow-y-auto bg-white text-[#1d1d1f] font-sans selection:bg-black selection:text-white pb-40
      ...
      >
        ...
        {/* Modal Lightbox ต้องสูงกว่าเนื้อหา แต่ยังไงก็ต้องสูงกว่า Navbar ถ้าอยากให้บัง Navbar ก็ใส่ z สูงๆ ได้ */}
        <div style={{ zIndex: 100000 }} className="fixed inset-0 bg-black/95 backdrop-blur-2xl hidden peer-checked:flex items-center justify-center transition-all duration-500 opacity-0 peer-checked:opacity-100">
            ...
            <div className="relative w-[95vw] h-[90vh] pointer-events-none transform scale-95 peer-checked:scale-100 transition-transform duration-500">
               {project.image && (
                  <Image src={project.image} alt={project.title} fill className="object-contain drop-shadow-2xl" priority />
               )}
            </div>
            <div className="absolute top-8 right-8 text-white/50 font-mono text-xs tracking-widest pointer-events-none">CLICK ANYWHERE TO CLOSE</div>
        </div>
        ...
        {/* --- MAIN CONTENT --- */}
        {/* เพิ่ม pt-32 (padding-top เยอะหน่อย) เพื่อให้เนื้อหาเริ่มใต้ปุ่มและ Navbar */}
        <main className="relative z-10 max-w-[90rem] mx-auto px-6 lg:px-12 pt-32">
           <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-24 items-start">
             ...
             <div className="lg:col-span-7 space-y-24">
               ...
               <label htmlFor="lightbox-toggle" className="cursor-zoom-in block relative group perspective-1000 my-8">
                   ...
                   <div className="relative w-full aspect-video md:aspect-[16/10] bg-white rounded-[2.5rem] border-2 border-black overflow-hidden shadow-[15px_15px_0px_0px_rgba(0,0,0,0.05)] transition-all duration-500 transform group-hover:translate-x-[-8px] group-hover:translate-y-[-8px] group-hover:shadow-[25px_25px_0px_0px_#f97316]">
                      {project.image ? (
                         <Image src={project.image} alt={project.title} fill className="object-contain p-10 lg:p-14 group-hover:scale-105 transition-transform duration-700 ease-in-out" priority />
                      ) : (
                         <div className="w-full h-full flex items-center justify-center font-black text-4xl text-gray-100 uppercase italic">No Visual</div>
                      )}
                      <div className="absolute inset-0 bg-black/5 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
                         <span className="bg-white px-8 py-3 rounded-full font-bold text-black border-2 border-black shadow-lg transform scale-0 group-hover:scale-100 transition-transform duration-300 tracking-widest">CLICK TO EXPAND</span>
                      </div>
                   </div>
               </label>
               ...
               <div className="relative pt-10">
                   ...
                   <h3 className="text-3xl font-black text-black mb-12 flex items-center gap-4 pl-4 border-l-8 border-orange-500">MISSION OBJECTIVES</h3>
                ...
                <div className="lg:sticky lg:top-36 animate-float">
                   <div className="space-y-6 mb-12">
                      <div className="flex flex-wrap gap-3">
                         <span className="px-4 py-2 bg-black text-white text-sm font-mono font-bold uppercase rounded-lg shadow-md transform -rotate-2">{project.category}</span>
                         <span className="px-4 py-2 bg-white border-2 border-black text-black text-sm font-mono font-bold rounded-lg shadow-md transform rotate-2">{project.year}</span>
                      </div>
                      <h1 className="text-6xl md:text-8xl font-black text-black leading-[0.9] tracking-tighter drop-shadow-sm uppercase">{project.title}</h1>
                   </div>
                   <div className="relative p-10 bg-white/90 backdrop-blur-md border border-gray-200 rounded-[2.5rem] shadow-sm hover:shadow-lg transition-shadow">
                      ...
                      <p className="text-gray-600 text-xl leading-loose font-medium text-pretty whitespace-pre-line">{project.fullDescription}</p>
                   </div>
                   <div className="space-y-6 pt-6">
                      ...
                      <div className="flex flex-wrap gap-4">
                         {project.stack.map((t, i) => (
                            <span key={i} className="px-6 py-3 bg-gray-50 rounded-2xl text-sm font-bold text-gray-600 border border-transparent hover:bg-white hover:border-black hover:text-black hover:shadow-[6px_6px_0px_#f97316] hover:-translate-y-1 transition-all cursor-crosshair">{t}</span>
                         ))}
                      </div>
                   </div>
                   {project.awards && (
                   ...
                   <Link href="/contact" className="group block w-full relative h-20 mt-12">
                      ...
                      <div className="absolute inset-0 bg-orange-600 rounded-3xl flex items-center justify-center border-4 border-black group-hover:-translate-y-1 group-hover:-translate-x-1 transition-transform duration-300">
                         <span className="font-black text-white text-2xl tracking-[0.2em] flex items-center gap-4">
                            CONTACT ME <svg className="w-8 h-8 animate-bounce" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={4} d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg>
                         </span>
                      </div>
                   </Link>
                </div>
